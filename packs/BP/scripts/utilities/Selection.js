...
async function onUse(player, block, item,property){
    ...
    if(Selection._blocksInUse.has(key) || Selection._playerInUse.has(player.id)) return;
    Selection._blocksInUse.add(key);Selection._playerInUse.add(player.id);
    const a = Selection.getSelection(player.id);
    if(Date.now() - a.lastModification > Selection.timeout){
        a.clear();
    }
    a.lastModification = Date.now();
    a[property] = block.location;
    const perm = block.permutation;
    world.playSound('bubble.pop',{location:block,pitch:0.6,volume:1});
    ...
    await sleep(5);
    Selection._blocksInUse.delete(key); Selection._playerInUse.delete(player.id);
}
