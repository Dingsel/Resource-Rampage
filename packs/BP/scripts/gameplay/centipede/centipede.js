...
//kills a centipede
function kill_centipede(centipedeidx){
...
//spawns a centipede
function summonCentipede(length,position){
    let centipede = []
...
const chatCallback = world.events.beforeChat.subscribe((eventData) => {
    if (eventData.message == "$summon centipede"){
        ...
        summonCentipede(10,eventData.sender.location)
...
function reach(s, e){
    ...
    return Vector.add(Vector.multiply(d.normalized(),0.75),s)
}
