...
async function aoeFire(level, location) {
    for (let i = 0; i < level; i++) {
        overworld.spawnParticle(`dest:aoe_${(level - 1 - i)}`, location, map)
        world.playSound("mob.ghast.fireball", { location })
        const targets = overworld.getEntities({ location, maxDistance: 6 + 5 * (level - 1 - i), excludeTypes: ["minecraft:player"] })
